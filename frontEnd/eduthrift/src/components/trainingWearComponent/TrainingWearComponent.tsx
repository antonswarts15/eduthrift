...
const trainingWearCategories: Record<string, { items: string[]; icon: string; color: string }> = {
  'Boys Training Wear': {
...
const TrainingWearComponent: React.FC<TrainingWearProps> = ({ userType, onItemSelect }) => {
  ...
  const getItemCount = (itemName: string) => {
    return listings.filter(listing =>
      ...
      listing.category === 'Training wear' &&
      listing.quantity > 0
  ...
  const getAvailableItems = () => {
    ...
    if (priceRange.max > 0 && priceRange.max < Math.max(0, ...listings.filter(l => l.category === 'Training wear').map(l => l.price))) items = items.filter(i => i.price <= priceRange.max);
          ...
          <>
            <div style={{ marginBottom: '16px' }}>
              <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', marginBottom: '10px' }}>
                {([{ label: 'All', value: undefined }, { label: 'Brand new', value: 1 }, { label: 'Like new', value: 2 }, { label: 'Used - good', value: 3 }, { label: 'Used - worn', value: 4 }] as { label: string; value: number | undefined }[]).map(c => (
                  <button key={c.label} onClick={() => setConditionFilter(c.value)} style={{
                    ...
                    backgroundColor: conditionFilter === c.value ? '#27AE60' : '#f0f0f0',
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <PriceRangeSlider
                  min={0}
                  max={Math.max(1, ...listings.filter(l => l.name === selectedItem && l.category === 'Training wear' && l.quantity > 0).map(l => l.price))}
                  value={priceRange.max === 0 ? { min: 0, max: Math.max(1, ...listings.filter(l => l.name === selectedItem && l.category === 'Training wear' && l.quantity > 0).map(l => l.price)) } : priceRange}
  ...
  return (
    <div>
      ...
      <IonAccordionGroup>
        {Object.entries(trainingWearCategories).map(([category, categoryData]) => (
