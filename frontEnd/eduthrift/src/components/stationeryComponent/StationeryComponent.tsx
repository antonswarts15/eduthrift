...
import stationeryPng from '../../assets/stationery.png';

interface StationeryComponentProps {
...
const stationeryCategories = {
  'Writing Instruments': {
    ...
    icon: pencilOutline, // TODO: replace with Writing Instruments PNG when asset is available
    color: '#004aad'
  },
  'Paper & Books': {
    ...
    icon: bookOutline, // TODO: replace with Paper & Books PNG when asset is available
    color: '#E74C3C'
  },
  'Drawing & Colouring': {
    ...
    icon: colorPaletteOutline, // TODO: replace with Drawing & Colouring PNG when asset is available
    color: '#27AE60'
  },
  'Measuring & Cutting': {
    ...
    icon: cutOutline, // TODO: replace with Measuring & Cutting PNG when asset is available
    color: '#F39C12'
  },
  'Adhesives': {
    items: ['Glue stick', 'White liquid glue', 'Sticky tape (clear)', 'Masking tape'],
    icon: pencilOutline, // TODO: replace with Adhesives PNG when asset is available
    color: '#8E44AD'
  },
  'Storage & Organisation': {
    items: ['Pencil case', 'Book covers (plastic)', 'Labels (self-adhesive)', 'School bag'],
    icon: bagOutline, // TODO: replace with Storage & Organisation PNG when asset is available
    color: '#1ABC9C'
  }
};

const StationeryComponent: React.FC<StationeryComponentProps> = ({ userType, onItemSelect, categoryFilter = 'all' }) => {
  // Buyer state
  ...
  const getFilteredItems = () => {
    let items = listings
      .filter(listing => listing.category === 'Stationery')
      .map(listing => ({
        id: listing.id,
        item: listing.name,
  ...
  const banner = (
    <div style={{ marginBottom: '16px', textAlign: 'center', backgroundColor: '#F39C12', borderRadius: '12px', padding: '16px' }}>
      <div style={{
        ...
        WebkitMaskImage: `url(${stationeryPng})`,
        maskImage: `url(${stationeryPng})`,
      ...
      <h2 style={{ margin: '0', color: 'white', fontSize: '18px', fontWeight: 'bold' }}>Stationery</h2>
      <p style={{ margin: '4px 0 0 0', color: 'rgba(255,255,255,0.85)', fontSize: '14px' }}>
        {userType === 'seller' ? 'Select the item to list' : 'School stationery & art supplies'}
      </p>
    </div>
  );
        ...
        <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', marginBottom: '10px' }}>
          {([
            ...
            <button key={c.label} onClick={() => setConditionFilter(c.value)} style={{
              ...
              backgroundColor: conditionFilter === c.value ? '#F39C12' : '#f0f0f0',
